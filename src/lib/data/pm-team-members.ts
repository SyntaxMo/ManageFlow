...
import type { CheckIn, Profile, Project, Task, WorkScheduleBlock } from "@/lib/db/types";
import {
...
export async function getPmTeamMembersPageData(
  ...
  const [checkInsTodayRes, historicalCheckInsRes, tasksRes, schedulesRes] =
    await Promise.all([
      supabase
        .from("check_ins")
        .select("*")
  ...
  if (checkInsTodayRes.error || historicalCheckInsRes.error) {
    attendanceLoadState = "error";
